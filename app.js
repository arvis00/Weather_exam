...
import { endpoint, gifEndpoint, forecastEndpoint, endpointCoord, endpointCoordOpenWeather} from './http/endpoints';
...
const showMoreBtn = () => {
  ...
  show.addEventListener('click', () => {
    ...
    if (cardsHeight > containerHeight) {
      document.querySelector('.cards-container').style.height = `${cardsHeight}px`;
...
function SendLocation(data) {
  ...
  const urlCoordCurrLoc = endpointCoord.getByCoord(latitude,longitude);
...
function coordResult(result){
  ...
  currentTemp.textContent = `${locTemp} ℃`
